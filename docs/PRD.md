...
| Módulos core | Los 28 módulos listados en §3 forman parte del MVP del framework. Su disponibilidad o visibilidad puede depender de configuración (`app_mode`, `billing_enabled`, permisos, plan), pero su contrato documental pertenece al MVP. |
...
7. **Construcción de la Aplicación:** Sobre la base del framework se construyen los módulos de negocio definidos en el PRD de la aplicación final, reutilizando los 28 módulos core y los mecanismos transversales (Grid Universal, Custom Fields, Audit Log, i18n, Import/Export).

**Módulos Iniciales (Bootstrap):** Durante la inicialización, el sistema pre-registra automáticamente en la tabla `modules` los **28 módulos core** del framework (listados en §3.A, §3.B, §3.C y §3.D). Estos módulos constituyen la infraestructura base del framework y deben estar disponibles desde el primer levantamiento. Post-inicialización, módulos adicionales de la aplicación derivada se registran en el Módulo de Módulos al momento de su creación, validando que el registro exista antes de implementar la lógica del módulo.
...
> [!IMPORTANT]
> **Referencia Técnica Canónica:** Esta sección opera como **índice ejecutivo**. La especificación técnica completa (schemas de BD, server actions, estructura de UI, integraciones y parámetros configurables) de los 28 módulos se encuentra en **[`REFERENCE_MODULES.md`](./REFERENCE_MODULES.md)**. Ambos documentos son complementarios: este PRD describe **qué** y **por qué**; el archivo de módulos especifica **cómo** se implementa técnicamente.

A continuación se lista la estructura de los 28 módulos del framework organizados por su bloque lógico.
...
| **Modelos AI** (`ai-model`)      | Catálogo configurable de proveedores y modelos de LLM disponibles para el Core de Integración IA (§4.2). Cada registro define un modelo con sus parámetros de inferencia, endpoint, costos para tracking, unidad/moneda de pricing, ventana de contexto, límites de entrada/salida, modalidades soportadas, timeout, deprecación y fallback. | Para abstraer proveedores, modelos y parámetros del Core AI sin imponer un proveedor final a las aplicaciones derivadas.                                                                    | Formulario CRUD para registrar proveedores y modelos.                                           |
...
| **Campos Personalizados** (`custom-field`) | Módulo que crea **definiciones de campos reutilizables** en formularios de otros módulos. 9 tipos de datos: `text`, `textarea`, `number`, `boolean`, `date`, `select`, `multi-select`, `email`, `url`. Almacena valores en columna JSONB `custom_data` de la entidad destino (sin EAV). Validación en cliente y Server Action. Permite **múltiples** definiciones; la cantidad de definiciones y de campos por módulo es **parametrizable por aplicación y por plan**. | Para adaptar la app a cualquier negocio permitiendo tipado fuerte a nivel UI sin migrar esquemas SQL. | UI para configurar campos y auto-renderizado en formularios. |
...
| **Webhooks** (`webhook`)                   | Webhooks **entrantes y salientes** con visibilidad de ambos. **Salientes:** emiten eventos HTTP firmados con HMAC-SHA256; retry con backoff exponencial (3 intentos); desactivación automática tras 10 fallos. **Entrantes:** endpoints receptores que verifican firma, timestamp y anti-replay. Allowlist de dominios/orígenes. | Para integrar sistemas de terceros en ambos sentidos con contratos de seguridad estrictos. | UI de configuración (entrante/saliente) con selector de eventos + historial de entregas. |
...
| **Plantillas Legales** (`legal-template`)  | Documentos legales **públicos** versionados (política de privacidad, términos, cookies) con editor **WYSIWYG a HTML**, por código, locale y jurisdicción. Las versiones publicadas son accesibles por usuarios autenticados y anónimos vía API; cada aceptación referencia la versión exacta en `consent_records`. | Para publicar y versionar documentos legales según la normativa del país, usables en web y móvil. | Editor WYSIWYG con historial de versiones y publicación. |
...
| **Modelos AI** (§3.A)    | `ai_models`    | Catálogo de proveedores registrados: nombre del modelo, proveedor (`provider`), endpoint base, parámetros de inferencia (`temperature`, `max_tokens` como máximo de salida), costos para cálculo de consumo (`cost_per_1k_tokens_input`, `cost_per_1k_tokens_output`, `pricing_unit`, `currency`), guardrails por petición (`context_window`, `max_input_tokens`, `max_cost_per_request`, `request_timeout_seconds`, `input_modalities`), streaming, deprecación (`deprecated_at`), fallback (`fallback_model_id`) y estado activo/inactivo. Admite **múltiples instancias por proveedor** (varias filas del mismo `provider` con `model_id` distintos). Los **topes de presupuesto acumulado** (tabla `ai_budgets`) se aplican por proveedor/modelo y por Tenant. |
...
- **Flujo de ejecución:**
  ...
  4. Consulta `ai_models` para obtener configuración del proveedor (endpoint, parámetros), validar `context_window`, `max_input_tokens`, `max_tokens` como máximo de salida, `max_cost_per_request`, `request_timeout_seconds` e `input_modalities`, y rechazar de forma controlada cualquier petición que exceda esos límites o que **superaría un tope de presupuesto activo** (`ai_budgets`, por proveedor/modelo o Tenant).
  5. Consulta `integrations` para obtener la API key del proveedor correspondiente al Tenant activo.
  6. Invoca al **Core AI** (runtime Python independiente, §6.3) mediante `fetch()` nativo a su API REST interna. El Core AI ejecuta la llamada al LLM externo utilizando la configuración del proveedor resuelta en los pasos 3-5. La comunicación se autentica con `CORE_AI_SECRET` (§6.3). Si el modelo principal falla, no está disponible o está deprecado, el motor puede usar `fallback_model_id` siempre que el fallback pase los mismos guardrails.
  7. Registra la invocación en el módulo Log (§3.A) como `ai.invocation` con metadata: `model_id`, `tokens_input`, `tokens_output`, `pricing_unit`, `currency`, `estimated_cost_usd` (ver §6.2 Tracking de Costos IA).
...
- **Integration Testing (Vitest):** Pruebas de Server Actions y APIs. Requieren base de datos de test local aislada por transacción (para evitar colisiones) o mediante Supabase Local. Validación de políticas RLS y consultas complejas. La validación de integración real (migraciones, RLS, Auth, Storage) se ejecuta además contra el entorno **Supabase cloud dev** según la jerarquía de entornos de datos (ver `DEPENDENCY_MATRIX.md`); "validado localmente" no equivale a "validado contra dev".
