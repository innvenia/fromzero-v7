...
- **Flujo de ejecución:**
  ...
  4. Consulta `ai_models` para obtener configuración del proveedor (endpoint, parámetros), validar `context_window`, `max_input_tokens`, `max_tokens` como máximo de salida, `max_cost_per_request`, `request_timeout_seconds` e `input_modalities`, y rechazar de forma controlada cualquier petición que exceda esos límites.
