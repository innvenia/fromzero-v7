...
| Historial de estados | 2026-06-21: creado durante Fase 1 para registrar activaciones pendientes; 2026-06-22: Sprint 9 implementó contrato Core AI local y mantuvo OpenRouter real diferido; 2026-06-22: Supabase Cloud dev verificado por conexión directa sin imprimir secretos; 2026-06-22: migración cloud Sprint 9 activada y validada con RLS negativa cross-tenant; 2026-06-22: plan pre-Sprint 10 aprobado, advisory RLS global de `settings`, `modules` y `plans` documentado como diferido aceptado sin migración |
...
| Advisory RLS global `settings`/`modules`/`plans` | diferido aceptado | El dueño eligió documentar sin migración antes de Sprint 10; tablas documentadas como globales sin `tenant_id`, pero el advisory remoto reporta RLS deshabilitado y `modules`/`plans` tienen grants de lectura para `authenticated` | Revisión de hardening en Sprint 11 o aprobación separada de migración/policies; confirmar matriz `settings` CRUD Super Admin, `modules` lectura limitada UI, `plans` plan propio/lectura limitada |
