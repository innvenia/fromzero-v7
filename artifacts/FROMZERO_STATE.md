...
- Bloqueos o riesgos: OpenRouter real sigue diferido; advisory preexistente detectó RLS deshabilitado en `public.modules`, `public.plans` y `public.settings`; el dueño aprobó documentarlo sin migración antes de Sprint 10 y revisarlo al finalizar todos los Sprints.
...
| Advisory RLS global `settings`/`modules`/`plans` | diferido aceptado hasta cierre de Sprints | cierre de Sprint 12, antes de release final | dueño aprobó documentar sin migración antes de Sprint 10 y revisar al finalizar todos los Sprints; tablas documentadas como globales sin `tenant_id`, con deuda de access-control/RLS para hardening |
...
- Bloqueos actuales: no iniciar Sprint 10 hasta cerrar PR pre-Sprint 10 con CI verde y working tree limpio; no cerrar release final sin revisar advisory RLS global diferido.
